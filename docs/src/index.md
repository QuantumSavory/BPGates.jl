...
```@warn
BPGates does not evolve full density matrices. Instead, states are represented in a twirled form in which off-diagonal density-matrix elements (coherences) are not tracked.

Consequently, simulations involving non-Pauli noise channels, such as T₁ (amplitude damping), correspond to the evolution of a twirled noise model rather than the exact physical channel. Results may therefore differ from those obtained using a full density-matrix simulator.

In particular, properties of the exact channel need not be preserved after twirling. Applying a noise process in several intermediate steps may produce a different result from applying the same process once for the combined duration.
```
